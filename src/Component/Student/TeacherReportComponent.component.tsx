...
import { Accordion, AccordionDetails, AccordionSummary, Box, Button, InputLabel, MenuItem, Select, SelectChangeEvent, Typography } from "@mui/material";
...
import { FamilyStatus, TeacherReport } from "../../Model/TeacherReport.model";
import { addTeacherReport, updateTeacherReport } from "../../Api/TeacherReport.api";
import { setAllStudents, updateStudent } from "../../Redux/Student/Student.Action";

const TeacherReportComponent: React.FC = () => {
    ...
    const [report, setReport] = useState<TeacherReport | null>(student?.teacherReport || null);
    const teacherReportTranslations = data.teacherReport;
    const dispatch = useDispatch();
    const MySwal = withReactContent(Swal);
    const [accordionStates, setAccordionStates] = useState<Record<string, boolean>>({});

    useEffect(() => {
        console.log("גיוח", student);
        debugger
        if (!student.teacherReport) {
            const newTeacherReport: TeacherReport = {
                id: "",
            ...
            setStudent((prevStudent) => ({
                ...prevStudent,
                teacherReport: newTeacherReport,
            }));
            setReport(newTeacherReport);
        }
        console.log("su", student);
    ...
    const handleSelectChange = (
        event: SelectChangeEvent<string>, // הערך מגיע כמחרוזת
        field: keyof TeacherReport
    ) => {
        if (report) {
            const numericValue = parseInt(event.target.value); // המרה למספר
            setReport({ ...report, [field]: numericValue });
        }
    };
    ...
    return (
        <div style={{ margin: "20px auto", maxWidth: "90%", fontFamily: "Arial, sans-serif" }}>
            ...
            {report && (
                ...
                >
                    {Object.entries(teacherReportTranslations).map(([key, value]) => (
                        <Accordion
                            ...
                            <AccordionDetails style={{ padding: "15px", backgroundColor: "#fafafa", borderRadius: "8px" }}>
                                {editable ? (
                                    <textarea
                                        ...
                                        onChange={(e) => handleInputChange(e, key as keyof TeacherReport)}
                                    />
                                ) : (
                                    <div
                                        style={{
                    ...
                    <Select
                        ...
                        id="student-type"
                        value={report?.familyStatus?.toString() || ""} // הערך הנוכחי (כמחרוזת)
                        onChange={(e) => handleSelectChange(e, "familyStatus")}
                        fullWidth
                        disabled={!editable}

                    >
                        {Object.entries(FamilyStatus)
                            .filter(([key, value]) => !isNaN(Number(value))) // מסנן רק ערכים מספריים
                            .map(([key, value]) => (
