...
//     //   if (!institutionName || !symbol || !managerName || !inspectorName || !contactPhone) {
//     // swal("הכנס את כל הנתונים ")        
//   //  return;
//   //     }
//     try {
...
const AddInstitutionForm: React.FC<AddInstitutionFormProps> = ({ onInstitutionAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // אם יש שגיאות, אל תשלח את הנתונים
      ...
      <Box my={4}>
        ...
        <form onSubmit={handleSubmit}>
          <TextField
            ...
            margin="normal"
            fullWidth
            error={errors.institutionName} // אם יש שגיאה, הצג את השגיאה
            value={institutionName}
            onChange={(e) => {
              setInstitutionName(e.target.value);
              setErrors({ ...errors, institutionName: false }); // נקה את השגיאה כשיש שינוי
            }}
          />
