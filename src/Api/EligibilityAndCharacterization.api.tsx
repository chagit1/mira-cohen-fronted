...
export const addMultiEligibilityStudent = (student: EligibilityAndCharacterization[]) => {
  const specialStudents = student.map((s) => ({
    ...s,
    birthDate: s.birthDate?.toISOString()
  }
  ));
  const response = axios.post(`${apiUrl}EligibilityAndCharacterization/AddMulti`, specialStudents);
  return response;
};
